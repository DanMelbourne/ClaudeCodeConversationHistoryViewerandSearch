...
### Done
- [x] Root-cause and fix the Sentry app-hang reports: superlinear inline markdown parsing, per-render re-parsing, O(n²) conversation row building, synchronous per-keystroke find, unbounded raw-JSON layout, byte-by-byte line counting, per-row `DateFormatter`
