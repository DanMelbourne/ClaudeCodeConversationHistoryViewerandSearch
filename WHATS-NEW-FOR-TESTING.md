...
## App-hang fixes — 2026-08-11

- [x] Automated: a 16 KB transcript line containing 500+ `snake_case` underscores parses in under 0.5 s (it took 7.9 s before; Sentry's hang threshold is 2 s).
- [x] Automated: unmatched `*`, `_`, `[` and `` ` `` delimiters do not backtrack — 18,000 characters of lone delimiters parse in under 0.5 s.
- [x] Automated: parse cost from 2 KB to 16 KB grows within a linear-ish bound, not quadratically.
- [x] Automated: headings, paragraphs, bullets, numbered items and fenced code blocks still parse correctly after the parser rewrite.
- [x] Automated: date dividers appear only after a gap over 30 minutes, and never on the first row.
- [x] Automated: building 20,000 conversation rows takes under 0.5 s.
- [x] Automated: in-conversation find matches text, thinking and tool-use blocks case-insensitively.
- [x] Automated: line counting matches the true newline count across read-buffer boundaries, and handles empty, missing and no-trailing-newline files.
- [x] Automated: oversized messages are truncated for display rather than laid out whole.
- [x] Verified: full suite 88/88 passing; `./build.sh` installed 1.0.7 (7); relaunched app samples with an idle main thread.
- [ ] **Not verified live — GUI automation permission was declined this session.** The checks below need a human.
- [ ] Open a long conversation (e.g. ScreenshotTray) and scroll fast top to bottom. It should stay smooth with no multi-second freezes.
- [ ] In a long conversation press the find bar and type quickly. Typing should never stall; results appear ~200 ms after you stop.
- [ ] Find a message containing `**bold** and \`code\`` on one line — the bold should render bold, not show literal asterisks. This was previously broken.
- [ ] Switch to Raw mode on a conversation where you pasted an image. It should show a bounded prefix ending in "… N more bytes"; Copy raw JSON should still put the whole record on the clipboard.
- [ ] Click between several large projects. The session list should populate faster than before.
- [ ] Leave the app open for a working day with Claude Code running, then check Sentry: app-hang events should stop arriving.
