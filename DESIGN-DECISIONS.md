...
## 2026-08-28: Publish one display-row projection per conversation change
**Decision**: `AppViewModel` produces its flat, date-divider-aware display rows off the main actor when it publishes a new conversation or the system-message setting changes. `ConversationView` renders that stored projection directly and never filters or rebuilds the complete conversation as part of `body`.
**Rationale**: Current Sentry app-hang reports repeatedly stop in SwiftUI's layout and transaction code, but they lack application symbols. Even after the earlier O(n²) repair, building a full row projection during every `body` evaluation is still O(n) work on the main actor and is amplified by scrolling, highlighting and unrelated UI state updates. Moving the one-pass projection to the explicit data-change boundary leaves render passes proportional to visible SwiftUI work.
**Verification**: Unit tests cover system-message filtering and date-divider placement at publication time, plus the existing 20,000-message linearity check. Production builds upload their dSYMs so any remaining reports identify the actual first-party frame.

## 2026-08-11: The build number is commit count plus an offset, from one shared helper
**Decision**: `scripts/build-number.sh` is the only place `CFBundleVersion` is computed — `BUILD = commit count + BUILD_NUMBER_OFFSET`, floored at `BUILD_NUMBER_FLOOR`. `build.sh` and `dist.sh` both source it. After any history rewrite that shortens the log, raise the offset by the number of commits removed and the floor to the highest build ever shipped.
**Rationale**: The commit count is monotonic only while history is append-only. Purging committed build output with `filter-repo` dropped 12 commits that became empty, cutting the count from 60 to 48 — the next build would have been numbered *below* the installed build 59. `build.sh` already had a `VERSION_FLOOR` hook anticipating this, but it was set to 1, and `dist.sh` had no protection at all, so a *release* could have carried a lower number than the development build it replaced. An offset beats a bare floor because it keeps one distinct number per commit instead of freezing every build at the floor until the count catches up.
**Verification**: `scripts/test_build_script.sh` now asserts the helper is the single source, that `dist.sh` does not re-derive the number, and that the computed value is at or above both the floor and the installed app's `CFBundleVersion`.

## 2026-08-11: Build output never enters git
**Decision**: `.gitignore` covers `dist/`, `build/`, `DerivedData/`, Xcode user state, `.DS_Store` and `.codegraph/`. The ~26 MB of xcarchives, dSYMs, signed binaries and a DMG already in history was purged with `git filter-repo` and force-pushed.
**Rationale**: The repository had no `.gitignore`, so an automated nightly backup branch had been committing whole build outputs since May — 13.8 MB of a public repo that was almost entirely regenerable artifacts. History was scanned for credentials first (tokens, keys, certificates, provisioning profiles): clean, so this was bloat rather than exposure, and the rewrite was safe to schedule rather than urgent. Force-pushing was acceptable because the repo had zero forks, zero stars and a single collaborator.
**Note**: The Sentry DSN in `Info.plist` is deliberately not treated as a secret — it is a write-only ingest key that ships inside every copy of the app.

## 2026-08-11: Inline markdown matching is one bounded pass, leftmost-wins
**Decision**: `MarkdownParser` resolves inline spans with a single left-to-right `matches(of:)` pass over one alternation. Every inner pattern is newline-free and length-capped (code 500, bold/italic 300–500, link 300/500). Parsed results are cached per distinct string in a main-actor `NSCache`, and per-render size guards use `utf8.count`, never `count`.
**Rationale**: The previous parser retried five *unbounded* regexes against the whole remaining string once per unmatched trigger character. Prose containing `snake_case` identifiers — routine in this app's content — made it O(triggers × length): a real 16 KB transcript line took 7,875 ms on the main thread, far past Sentry's 2 s app-hang threshold. Bounding the inner patterns means a lone delimiter fails inside a fixed window instead of backtracking to the end of the line; the same line now costs 28 ms. Leftmost-wins also *fixes* a rendering bug — the old global code-span-first ordering made `**bold** and \`code\`` emit the bold markers literally.
**Rejected**: raising the existing 50,000-character bail-out. It was never the mechanism — the cost was per trigger character, so a 4 KB line already blocked for 851 ms while sitting well under the guard.

## 2026-08-11: Nothing derived per row may be O(n) in the message list
**Decision**: A SwiftUI row body must never read a computed property that scans the whole collection. `ConversationView` builds `[MessageRow]` — message plus resolved date-divider flag — in one pass before the `ForEach`, and the local find runs debounced and off the main actor.
**Rationale**: SwiftUI evaluates `body` on every scroll tick and every unrelated state change, so an O(n) read inside a row body is O(n²) per render pass. This is the same class of mistake as the eagerly-built nested `LazyVStack` already recorded in `CLAUDE.md`; the fix is the same shape — flatten and precompute into one typed array.
